...
public class EnseignantDaoImpl implements IEnseignantDao{
	...
	public boolean checkIfValidEnseignant(Long id) {
	    // Query the database to check if the idEnseignant exists
	    // This assumes that you have a database connection object called "conn"
		Connection conn=SingletonConnection.getConnection();
	    try {
	    	PreparedStatement ps= conn.prepareStatement( "SELECT COUNT(*) FROM enseignants WHERE id = ?");
	    	ps.setLong(1, id);
	        ResultSet rs = ps.executeQuery();
	        rs.next();
